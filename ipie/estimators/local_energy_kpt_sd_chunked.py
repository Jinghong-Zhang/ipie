...
from ipie.estimators.kernels.gpu.exchange_kpt import exx_kpt_kernel
...
def local_energy_kpt_single_det_uhf_batch_chunked_gpu(
    ...
    # ghalfa = ghalfa.transpose(1, 3, 0, 2, 4).copy() # nk, nk, nw, nalpha, nbasis
    # ghalfb = ghalfb.transpose(1, 3, 0, 2, 4).copy() # nk, nk, nw, nbeta, nbasis
    # ghalfaTx = ghalfa.transpose(3, 1, 4, 2, 0).copy() # nk, nk, nbasis, nalpha, nw
    # ghalfbTx = ghalfb.transpose(3, 1, 4, 2, 0).copy() # nk, nk, nbasis, nbeta, nw
    ghalfa_send = ghalfa.transpose(1, 3, 0, 2, 4).copy()
    ghalfb_send = ghalfb.transpose(1, 3, 0, 2, 4).copy()
    # ghalfaTx_send = ghalfa.transpose(3, 1, 4, 2, 0).copy()
    # ghalfbTx_send = ghalfb.transpose(3, 1, 4, 2, 0).copy()

    ghalfa_recv = xp.zeros_like(ghalfa_send)
    ghalfb_recv = xp.zeros_like(ghalfb_send)

    # ghalfaTx_recv = xp.zeros_like(ghalfaTx_send)
    # ghalfbTx_recv = xp.zeros_like(ghalfbTx_send)
