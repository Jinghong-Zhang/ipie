...
def make_ham(relaxed_like, seed=401):
    ...
    return HamTangent(NOCC, NAO, h1e, chol, enuc, dh1e=obs_mat, dchol=dchol), psi


def run_path(ham, psi, content, sr_replay=None):
    ...
    E, dE, W, dW, _ = driver.run_along_path(
        ham, trial, NUM_MEASUREMENTS, sr_replay=sr_replay
    )
...
def test_along_path_gradient_vs_frozen_sr_fd(relaxed_like):
    ...
    E0, dE, W0, dW, sr_record, diag0 = run_path(ham, psi, content)
    ...
    for eps in (1e-4, 5e-5):
        ...
        Ep, _, Wp, _, _, diag_p = run_path(shifted_copy(ham, eps), psi, content, sr_record)
        Em, _, Wm, _, _, diag_m = run_path(shifted_copy(ham, -eps), psi, content, sr_record)
