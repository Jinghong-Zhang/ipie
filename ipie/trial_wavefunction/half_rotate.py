...
def half_rotate_spin(
    ...
    chol = hamiltonian.chol.reshape((M, M, nchol))
    h1b = hamiltonian.H1[1]
    chol_beta = chol
    orbsb_work = orbsb

    if isinstance(hamiltonian, GenericRealChol):
        mo_coeffb = getattr(trial, "mo_coeffb", None)
        if mo_coeffb is not None:
            if hamiltonian.h1eb is None or hamiltonian.cholb is None:
                hamiltonian.construct_beta_integrals(mo_coeffb)
            h1b = hamiltonian.h1eb
            chol_beta = hamiltonian.cholb
            eye_beta = np.eye(M, nb, dtype=orbsb.dtype)
            orbsb_work = np.broadcast_to(eye_beta, (ndets, M, nb))
    ...
    rH1a[:] = np.einsum("Jpi,pq->Jiq", orbsa.conj(), hamiltonian.H1[0], optimize=True)
    rH1b[:] = np.einsum("Jpi,pq->Jiq", orbsb_work.conj(), h1b, optimize=True)
    ...
    if compute:
        if isinstance(hamiltonian, GenericComplexChol):
            L_a = [chol, cholbar, A, B]
            L_b = [chol, cholbar, A, B]
        elif isinstance(hamiltonian, GenericRealChol):
            L_a = [chol]
            L_b = [chol_beta]
